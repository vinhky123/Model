...
class Model(nn.Module):
    """
    GRU Model with support for all 5 tasks:
    - Long-term/Short-term Forecasting: Encoder-Decoder GRU with Attention
    - Imputation: Bidirectional GRU for context-aware reconstruction
    - Anomaly Detection: GRU Autoencoder for reconstruction
    - Classification: GRU Encoder + Global Pooling
    
    GRU is similar to LSTM but simpler (no cell state, only hidden state)
    """
    ...
    def __init__(self, configs):
        ...
        if self.task_name in ["long_term_forecast", "short_term_forecast"]:
            ...
            # GRU Encoder for anomaly detection
            self.encoder = nn.GRU(
                input_size=configs.d_model,
                hidden_size=configs.d_model,
                num_layers=configs.e_layers,
            ...
            # GRU Encoder for classification
            self.encoder = nn.GRU(
                input_size=configs.d_model,
                hidden_size=configs.d_model,
                num_layers=configs.e_layers,
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        """
        Encoder-Decoder GRU with Attention for forecasting
        """
        ...
        encoder_outputs, encoder_hidden = self.encoder(enc_out)
        
        # Decoder
        dec_out = self.dec_embedding(x_dec, x_mark_dec)
        
        # Transfer encoder's final state to decoder
        # Repeat last layer's hidden state for all decoder layers
        decoder_hidden = encoder_hidden[-1:].repeat(self.decoder.num_layers, 1, 1)
        
        decoder_outputs, _ = self.decoder(dec_out, decoder_hidden)
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        """
        Bidirectional GRU for imputation task
        """
        # Normalization
        ...
        # Bidirectional GRU
        gru_out, _ = self.encoder(enc_out)
    ...
    def classification(self, x_enc, x_mark_enc):
        """
        GRU Encoder with Global Pooling for classification
        """
        # Embedding
        enc_out = self.enc_embedding(x_enc, None)
        
        # GRU Encoder
        gru_out, hidden = self.encoder(enc_out)
