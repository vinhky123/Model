...
from models.MambaSimple import MambaBlock, RMSNorm
import math
...
class MambaLayer(nn.Module):
    """
    Wrapper for MambaBlock to match AttentionLayer interface
    AttentionLayer expects: forward(q, k, v, attn_mask, tau, delta) -> (output, None)
    MambaBlock expects: forward(x) -> output
    """
    ...
    def forward(self, q, k, v, attn_mask=None, tau=None, delta=None):
        """
        Match AttentionLayer interface
...
class EncoderLayer(nn.Module):
    """
    EncoderLayer with Mamba replacing self-attention
    Original: self_attention (FlashAttention) + cross_attention (STAR_patch)
    Modified: mamba_layer (Mamba) + cross_attention (STAR_patch)
    """
    def __init__(
        self,
        mamba_layer,      # ← Mamba replaces self_attention
        cross_attention,  # ← Keep STAR_patch
        d_model,
        d_ff=None,
        dropout=0.1,
        activation="relu",
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None, tau=None, delta=None):
        ...
        # Mamba layer (replacing self-attention)
        x = x + self.dropout(
            self.mamba_layer(x, x, x, attn_mask=x_mask, tau=tau, delta=None)[0]
        )
        x = self.norm1(x)
        ...
        # STAR cross-attention (unchanged)
...
class Model(nn.Module):
    ...
    Architecture:
    - Replaces O(L²) FlashAttention with O(L) Mamba
    - Keeps STAR_patch cross-attention module
    - Maintains patch-based embedding
    - Uses same forecasting head
    
    Benefits:
    - Linear time complexity O(L) vs quadratic O(L²)
    ...
    def __init__(self, configs):
        ...
        # Encoder-only architecture with Mamba
        self.encoder = Encoder(
            [
                EncoderLayer(
                    MambaLayer(configs),  # ← Mamba instead of AttentionLayer
